...
with intro_col:
    st.write(
        ...
        "ranking narrowed by a text-match filter — all/any/exact-phrase — "
        "on the article body), and **Lucene** (raw `query_string` for "
        "boost / slop / phrase prefix / fuzzy / regex / cross-field "
        "queries)."
    )
    st.write(
...
with tab_combined:
    ...
    st.markdown(
        ...
        "similarity to your description (Gemini-2). Pinecone has three "
        "text-match filter operators, each a different precision/recall "
        "trade-off — pick one below:\n\n"
        ...
        "same thing together.\n\n"
        "**The demo flip**: visual-only `black bird with bright spots on "
        "wings` ranks the **Red-winged blackbird** #19 — outside the top "
        "10. Filter on `epaulet, yellow` with **All terms** and it leaps "
        "to **#1** (its red shoulder patch — an \"epaulet\" — and yellow "
        "wing bar are exactly what the visual query describes). Switch "
        "the *same two terms* to **Any term** and it falls back to **#7** "
        "— one term alone can't hold the candidate pool to birds that are "
        "actually a strong match on both signals. **Exact phrase** on "
        "`yellow wing bar` (the literal phrase from the article) also "
        "puts it back at #1."
    )
