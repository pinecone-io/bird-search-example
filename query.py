...
def search_filter_visual(
    ...
    candidates, then dense-vector similarity to ``visual_q``'s embedding
    (local SigLIP by default, or Gemini — see embedder.py) ranks the
    survivors. One Pinecone call — hard filter + visual rerank.
    ...
    Example (verified live): ``filter_terms=["epaulet", "yellow"]`` on
    ``body`` with ``mode="all"`` plus ``visual_q="black bird with bright
    spots on wings"`` puts the Red-winged blackbird at #1 (visual-only
    search alone ranks it #19). Switching to ``mode="any"`` on the *same*
    terms drops it to #7 — the wider net admits enough other yellow- or
